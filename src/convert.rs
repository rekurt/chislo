...
mod tests {
    ...
    fn test_int_to_words_basic() {
        let cases = [
            ...
            (987654, "девятьсот восемьдесят семь тысяч шестьсот пятьдесят четыре"),
        ];
    ...
    fn test_int_to_words_negative_and_spacing() {
        ...
        assert_eq!(
            convert_int_to_words(-1, Gender::Masculine),
            "минус один"
        );
        ...
        for n in [0, 1, 100, 1000, 987654] {
            ...
            assert_eq!(
                words,
                words.trim(),
                "Trailing space in int_to_words({n})"
            );
        }
    }
    ...
    fn test_int_to_words_gender() {
        assert_eq!(
            convert_int_to_words(42, Gender::Masculine),
            "сорок два"
        );
        assert_eq!(
            convert_int_to_words(42, Gender::Feminine),
            "сорок две"
        );
    ...
    fn test_thousands_always_feminine() {
        ...
        assert_eq!(
            convert_int_to_words(1000, Gender::Masculine),
            "одна тысяча"
        );
        assert_eq!(
            convert_int_to_words(2000, Gender::Masculine),
            "две тысячи"
        );
        assert_eq!(
