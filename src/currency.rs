...
pub fn money(whole: i64, cents: u32, currency: &Currency) -> String {
    ...
    let cents_decl = get_declension(cents as i64, currency.frac.0, currency.frac.1, currency.frac.2);
...
pub fn money_from_str(amount: &str, currency: &Currency) -> Result<String, crate::Error> {
    ...
    let whole: i64 = parts[0].parse().map_err(|_| crate::Error::InvalidNumber(format!("invalid amount: '{}'", parts[0])))?;
    let cents = if parts.len() > 1 { parse_cents(parts[1])? } else { 0 };
...
fn parse_cents(frac_str: &str) -> Result<u32, crate::Error> {
    let s = if frac_str.len() >= 2 {
        ...
        return frac_str[..1].parse::<u32>().map(|d| d * 10).map_err(|_| crate::Error::InvalidNumber(format!("invalid cents: '{frac_str}'")));
    } else {
        return Ok(0);
    };
    s.parse::<u32>().map_err(|_| crate::Error::InvalidNumber(format!("invalid cents: '{frac_str}'")))
}
...
mod tests {
    ...
    #[test]
    fn test_money_rub() {
        assert_eq!(money(1234, 56, &RUB), "одна тысяча двести тридцать четыре рубля пятьдесят шесть копеек");
        assert_eq!(money(1, 1, &RUB), "один рубль одна копейка");
    }

    #[test]
    fn test_money_from_str() {
        assert_eq!(money_from_str("1234.56", &RUB).unwrap(), "одна тысяча двести тридцать четыре рубля пятьдесят шесть копеек");
    }
}
